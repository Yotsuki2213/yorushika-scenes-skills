...
**人物是故事的一部分。** 没有人物主体，就添一个青年或少女白色线稿人物，而且必须赤脚：动作先服从场景的路径、边缘、支撑和视线，只有缺少更强线索时才自然地背身回望；身体和脚用较粗、断续、潦草的试画线和少量排线表达，脚部只留简单落点线，保留大量透底，但重心与落点仍符合真实场景。已有人物主体，则保留身体、衣着、鞋袜、姿态和位置，只用密集白线覆盖整个可见头部，连头发、耳朵和后脑也藏进笔触里。背景路人不自动算主体，画外的头部不会被补画。详见[人物处理与参考](skills/yorushika-mv-scenes/references/human-treatment.md)。
...
纸色取自画面，轻微做旧。
先分析真实 MV 图，再从完整 opus.md 感性匹配并核对日中歌词与原歌名，
按图片横竖方向排版，核对完成后直接生成。
```

一张普通照片，会先走完 MV 阶段；已经生成好的 MV 图，则直接复用。真正的画面准备好之后，才开始为它寻找文字：

**获得 MV 图 → 分析元素、构图与情绪 → 核对歌词和歌名 → 编排生成 → 保存**

| MV 图片方向 | 明信片布局 | 默认歌词 |
| --- | --- | --- |
| 横向／正方形 | 图片居中偏上；下方用日文一行、中文一行，歌名另列 | **自动选 1–2 组** |
...
横向／正方形会把1–2组短歌词压缩成两条正文行：第一行按原顺序放日文，第二行放对应中文；两组放不下时自动回退为一组。歌名另起一行，不计入这两行：
...
以上是排版示意，不是待生成的实际文案。技能会从完整 [opus.md](skills/yorushika-postcard-scenes/references/opus.md) 跨发行分组召回候选，再按画面的情绪张力、构图关系、感官意象、语义余韵和版面长度重排，而不是停在最先命中的几首歌。最终文字仍来自同一曲目、同一条目，保留原词、标点与日中配对，不拼接不同译本。助手核对完成后再交给图像工具排版，默认不额外等待确认；想先看配文，只要说“先给我看歌词”。

明信片为已经完成的 MV 做更明显的边缘溶图与纸面排版：约占图片短边 1–4% 的外围可以连续淡化、渗墨、干刷并融入纸张。线稿人物的轮廓、四肢、承重点、接触关系、人物动作和所有白线始终受到保护；人物碰到边缘时，溶图会绕开它或只向外扩散。

## 今天想留多少话？

不用记住所有参数，直接说你的偏好就好：

- **“这次只留画面。”** 一般的“不新增文字”请求会关闭新增歌词与歌曲出处；logo 是每张明信片的固定组成，仍然保留。
- **“保留 logo，不放歌词。”** 使用 `lyrics=none`，同时关闭歌曲出处。
- **“保留歌词，不放 logo。”** 仅显式的 `signature=none` 会去掉 logo；默认每张明信片都带署名。
- **“竖图这次也只要两组。”** 显式 `lyric_lines=2` 覆盖默认数量；支持 1–4 组。
- **“横图这次要两句。”** 横版会把两组日文合成一行、对应中文合成下一行；若明确要求3–4组，则不再保证正文只有两行。
...
└── skills/
    ...
    │   ├── SKILL.md / agents/
    │   ├── references/
    │   └── assets/line-figures/   透明线稿母版、纯线稿动作母版与原始参考
    └── yorushika-postcard-scenes/ 纸张、横竖布局与双语选句
        ├── SKILL.md / agents/
        ├── references/           含 opus.md 与按方向加载的版式模块
        ├── scripts/              全库歌词候选提取器
