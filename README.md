...
**人物是故事的一部分。** 没有人物主体，就添一个略抽象、潦草的白色线稿人物：默认背对镜头，自然地越肩回头，当该动作与场景的视线、路径、支撑或叙事矛盾时，可自行调整为同样贴合场景的可信动作；脚步、重心与落点都符合真实场景。已有人物主体，则保留身体、衣着、姿态和位置。两种情况都用密集白线覆盖整个可见头部，连头发、耳朵和后脑也藏进笔触里。背景路人不自动算主体，画外的头部不会被补画。详见[人物处理与参考](skills/yorushika-mv-scenes/references/human-treatment.md)。
...
└── skills/
    ...
    │   ├── SKILL.md / agents/
    │   ├── references/
    │   └── assets/line-figures/   轻量联系表与三张原始线稿参考
