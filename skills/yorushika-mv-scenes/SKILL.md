...
- Output one raster image using built-in `image_gen` and return it with a brief inline summary and absolute saved path. Keep the full production prompt and working analysis internal; do not create a production-record Markdown file unless the user explicitly requests one.
...
Return the image with a brief inline summary of the resolved settings and absolute saved path: `transform_mode`, `preserve_strength`, `style_intensity`, route/weights, `source_orientation`, `target_aspect`, `human_subject_present`, `human_treatment`, figure placement/depth/scale basis/contact or head-coverage choice, key anchors/locks, effect zones and microcopy/gloss if any. Keep the full production prompt and detailed notes in working context only; do not write a production-record Markdown file by default. Create a Markdown record only when the user explicitly asks for production notes, a handoff, a review, or a reproducible record.
