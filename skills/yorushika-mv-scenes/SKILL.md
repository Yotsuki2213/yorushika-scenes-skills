...
3. Read the shared [human line grammar](references/human-treatment.md), then exactly one module from each applicable group:
   - Human branch: [add a protagonist](references/humans/add-white-protagonist.md) or [cover existing heads](references/humans/cover-existing-heads.md).
...
5. When line treatment is needed, inspect the bundled [transparent line-figure master](assets/line-figures/线稿人物场景融合母版.png) and attach it as `SUPPORTING LINE STYLE AND FIGURE-CONTACT REFERENCE`; attach the user's image as `EDIT TARGET`. It supplies rough line rhythm, youthful posture, head anonymity and simple barefoot contact marks—not a fixed pose, outfit, prop, background or collage layout. Every newly added youth/girl is completely barefoot; feet are only a few coarse strokes tied to the real support. The pure-line action master, older contact sheet and originals are archival fallbacks for diagnosing style regression and are not normal generation inputs.
6. Generate one raster image with built-in `image_gen` and save it as described below.

## Non-negotiable image behavior

- No human subject: add one legible late-teen or young-adult youth or girl in rough pure-white lines, always completely barefoot. Choose the action from the scene's path, edge, support, sightline and emotional distance; use a natural back-facing look-back only when the scene offers no stronger cue. Keep joints, balance, weight-bearing contact, perspective and occlusion plausible. The body and feet use coarse broken contours, a few searching strokes and sparse hatching with large transparent gaps; feet are simple marks, never realistic toes, soles or footwear. The entire head is anonymized by dense mostly-horizontal white scribbles.
