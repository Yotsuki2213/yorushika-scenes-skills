...
human_subject_details: primary subjects' positions, actions, protected bodies, entire visible head regions; if adding, use a resolved figure plan for location, depth, scale, action, support/contact and occlusion
figure_plan: location, depth plane, scale basis, action, support_contact, occlusion, and eye_path for the added figure; internal production data only
...
3. Apply the resolved human branch: if no human subject exists, place one legible **pure-white** sketched protagonist only after resolving its scene-supported location, depth plane, scale basis and contact; then choose the action, with the back-facing torso and natural over-shoulder look back used when physically compatible. A distant figure may remain a small part of the frame while its silhouette, action direction and head scribble stay readable. Otherwise preserve existing subjects' bodies/poses and obscure their entire visible heads with dense white hatching and horizontal scribbles. A head outside the frame is not added. The branch is common to all routes.
...
- for a new protagonist: a scene-derived location and depth plane, scale matched to perspective and nearby references, believable joints and support, a compatible action, slightly abstract scrawled contours and hatching with body gaps, and dense coverage of the entire visible head;
