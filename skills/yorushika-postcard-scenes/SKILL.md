...
- Do not create a production-record Markdown file by default. Keep scene analysis, lyric matching evidence, layout calculations and prompt details in the current working context; write a `.md` record only when the user explicitly requests production notes, a handoff, a review, or a reproducible record.
...
Before calling ImageGen for the postcard, resolve in the current working context (not a Markdown file) the scene analysis, retrieval terms and corpus coverage, orientation/layout, selected pair count, exact Japanese and Chinese strings with source line numbers, song/translator metadata, emotional/compositional matching reason, layout rows and complete `——original song title` string. Read [art direction](references/postcard-art-direction.md) to ensure the chosen excerpt fits. If fit requires reselection, prefer a shorter window from the same song, then the next emotionally strongest candidate, and verify the new excerpt before compiling.
...
Return the image with its reused/generated MV source, layout, paper/blend choices, figure preservation, selected bilingual excerpt, original song title and absolute saved path. Keep detailed analysis, prompts, source line numbers, translator metadata and matching rationale in working context only; do not create a production-record Markdown file by default. Create one only when the user explicitly requests production notes, a handoff, a review, or a reproducible record.
